...
adsl_ht_wt <- adsl %>%
  filter(EFFFL == 'Y') %>%
  select(USUBJID, TRT01AN, TRT01A, SEX, HEIGHTBL, WEIGHTBL)

head(adsl_ht_wt, 10)
```

```{r}
# SAMPLE 1 - Using pivot longer to construct HEIGHTBL and WEIGHTBL in a single column as parameters with their values

samp1_ht_wt <- adsl_ht_wt %>%
  pivot_longer(cols = c(HEIGHTBL, WEIGHTBL),
               names_to = "HT_WT",
               values_to = "value")
...
```{r}
samp1_ht_wt_stat <- samp1_ht_wt %>%
  group_by(TRT01AN, TRT01A, SEX, HT_WT) %>%
  summarize(mean = mean(value) %>% round(digits = 2) %>% format(nsmall=2),
            sd = sd(value) %>% round(digits = 2) %>% format(nsmall = 2), 
            med = median(value) %>% round(digits = 2) %>% format(nsmall = 2),
            min = min(value) %>% round(digits = 2) %>% format(nsmall = 2), 
            max = max(value) %>% round(digits = 2) %>% format(nsmall = 2),
            n = n() %>% format(nsmall = 0)) %>%
  mutate(range_minmax= paste0("(",min, ",", max, ")")) %>%
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
  mutate(HT_WT = case_when(HT_WT == "WEIGHTBL" ~ "Weight (kg)",
                           HT_WT == "HEIGHTBL" ~ "Height (cm)"))
samp1_ht_wt_stat
```

```{r}
samp1_desc_stat_long <- samp1_ht_wt_stat %>%
  ungroup() %>%
  select("TRT01A", "SEX", "HT_WT", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","HT_WT","SEX"), names_to ="category", values_to = "values" )

samp1_desc_stat_long
```
...
```
```{r}
# SAMPLE 2 - Getting summary statistics per HEIGHTBL & WEIGHTBL to show the use of the function

samp2_ht_stat <- adsl_ht_wt %>%
  group_by(TRT01AN, TRT01A, SEX) %>%
  summarize(mean = mean(HEIGHTBL) %>% round(digits = 2) %>% format(nsmall=2),
            sd = sd(HEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2), 
            med = median(HEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2),
            min = min(HEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2), 
            max = max(HEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2),
            n = n() %>% format(nsmall = 0)) %>%
  mutate(range_minmax= paste0("(",min, ",", max, ")")) %>%
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
  mutate(PARAM = "Height (cm)")
samp2_ht_stat
```

```{r}

samp2_wt_stat <- adsl_ht_wt %>%
  group_by(TRT01AN, TRT01A, SEX) %>%
  summarize(mean = mean(WEIGHTBL) %>% round(digits = 2) %>% format(nsmall=2),
            sd = sd(WEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2), 
            med = median(WEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2),
            min = min(WEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2), 
            max = max(WEIGHTBL) %>% round(digits = 2) %>% format(nsmall = 2),
            n = n() %>% format(nsmall = 0)) %>%
  mutate(range_minmax= paste0("(",min, ",", max, ")")) %>%
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
  mutate(PARAM = "Weight (kg)")
samp2_wt_stat
```

```{r}

samp2_ht_wt_stat <- rbind(samp2_ht_stat, samp2_wt_stat)
samp2_ht_wt_stat
```
```{r}
samp2_desc_stat_long <- samp2_ht_wt_stat %>%
  ungroup() %>%
  select("TRT01A", "SEX", "PARAM", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","PARAM","SEX"), names_to ="category", values_to = "values" )

samp2_desc_stat_long
```
...
```{r}
# Using a function for Weight & Height

varSummary_ht_wt <- function(.data = NULL, variable = NULL, roundby = NULL, param = NULL) {
  .data %>%
    group_by(TRT01AN,TRT01A,SEX) %>%
    summarize(mean = mean({{variable}}) %>% round(digits = {{roundby}}+2) %>% format(nsmall = {{roundby}}+2),
              sd = sd({{variable}}) %>% round(digits = {{roundby}}+2) %>% format(nsmall = {{roundby}}+2), 
              med = median({{variable}}) %>% round(digits = {{roundby}}+2) %>% format(nsmall = {{roundby}}+2),              
              min = min({{variable}}) %>% round(digits = {{roundby}}+2) %>% format(nsmall = {{roundby}}+2), 
              max = max({{variable}}) %>% round(digits = {{roundby}}+2) %>% format(nsmall = {{roundby}}+2),
              n = n() %>% format(nsmall = {{roundby}}), 
              .groups = "keep") %>%
    mutate(range_minmax= paste0("(",min, ",", max, ")")) %>%
    mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
    mutate(PARAM = {{param}})
...
ht_stat_func <- adsl_ht_wt %>%
  varSummary_ht_wt(variable=HEIGHTBL, roundby=0, param = "Height (cm)")

ht_stat_func
```

```{r}
wt_stat_func <- adsl_ht_wt %>%
  varSummary_ht_wt(variable=WEIGHTBL, roundby=0, param = "Weight (kg)")

wt_stat_func
```

```{r}
ht_wt_stat_func <- rbind(ht_stat_func, wt_stat_func) 

ht_wt_stat_func
```

```{r}
desc_stat_long_func <- ht_wt_stat_func %>%
  ungroup() %>%
  select("TRT01A", "SEX", "PARAM", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","SEX","PARAM"), names_to ="category", values_to = "values" )
