...
agestat_header <- tribble(~category,
                          "Age (Years)")

comb_age <- bind_rows(agestat_header, agestat_cat_N, age_cat_order, agestat_cat_noN)
comb_age
```
...
cnt_perc <- function(data, var, varn=NULL) {
  ...
  if (!is.null(varn)) {
      var_cat <- var_cat %>% arrange(.data[[varn]])
  }
  
  # Add Header for the Variable Stats
  var_header <- tribble(~category, str_to_title(var))
  
  # Combine Header with Data and Return Result
  comb_var <- bind_rows(var_header, var_cat) %>%
      select(-all_of(varn))
